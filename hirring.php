 ...
 if (isset($_GET['search'])) {
    ...
    $sql = "SELECT * FROM `t_compania` c,`t_empleos` j WHERE c.`COMPANYID`=j.`COMPANYID` AND COMPANYNAME LIKE '%" . $COMPANYNAME ."%' ORDER BY DATEPOSTED DESC" ;
    $mydb->setQuery($sql);
 ...
 // }
 //    $sql = "SELECT * FROM `t_compania` c,`t_empleos` j WHERE c.`COMPANYID`=j.`COMPANYID` AND c.COMPANYID LIKE '%" . $companyid ."%' ORDER BY DATEPOSTED DESC" ;
 //    $mydb->setQuery($sql);
 //    $cur = $mydb->loadResultList();
