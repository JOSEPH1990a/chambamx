							  ...
							  <tbody>
							  	<?php   
							  		// $mydb->setQuery("SELECT * 
											// 			FROM  `t_usuarios` WHERE TYPE != 'Customer'");
							  		$mydb->setQuery("SELECT * FROM `t_compania` c  , `t_empleos_aplicados` j, `t_empleos` j2, `t_candidatos` a WHERE c.`COMPANYID`=j.`COMPANYID` AND  j.`JOBID`=j2.`JOBID` AND j.`APPLICANTID`=a.`APPLICANTID` ");
