...
function doInsert($jobid=0,$fileid=0) {
	...
	global $mydb; 
			...
			$sql = "SELECT * FROM `t_compania` c,`t_empleos` j WHERE c.`COMPANYID`=j.`COMPANYID` AND JOBID = '{$jobid}'" ;
			$mydb->setQuery($sql);
...
function doRegister(){
	...
	if (isset($_POST['btnRegister'])) { 
			...
			if ($age < 20){
			message("Edad inválida. Solo se permiten mayores de 18 años.", "error");
			...
			message("Te has registrado correctamente en el Portal. Puedes iniciar sesión ahora!","success");
...
function doLogin(){
  ...
  //it creates a new objects of member
    ...
    if ($res==true) { 
       	...
       	message("Éxito, ha iniciado sesión correctamente!","success");
